...
pub fn get_number(object: &JsonObject, key: &str) -> Result<f64, JsonError> {
    object
        .get(key)
...
pub fn get_string_array(object: &JsonObject, key: &str) -> Result<Vec<String>, JsonError> {
    let array = object
        .get(key)
    ...
    array
        ...
        .map(|value| {
            value
                .as_str()
                .map(ToOwned::to_owned)
                .ok_or_else(|| JsonError::new(format!("array field `{key}` must contain strings")))
        })
        .collect()
}
...
pub fn get_optional_value(object: &JsonObject, key: &str) -> Option<JsonValue> {
